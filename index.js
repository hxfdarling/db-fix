...
async function decrypt(file, password) {
	...
	if (await fse.exists(file)) {
		...
		await fse.writeFile(decryptedFile, Buffer.concat([data, decipher.final()]));
	}
...
async function encrypt(file, password) {
	...
	if (await fse.exists(file)) {
		...
		await fse.writeFile(encryptFile, Buffer.concat([data, cipher.final()]));
	}
	return encryptFile
};

async function exportSQL(file) {
	let child = spawn('./lib/sqlite3.exe', [file, '.dump'], {
		cwd: __dirname
	})
	let result = ''
	let resolve
	...
	child.stdout.on('close', async() => {
		...
		await fse.writeFile(tmp, result)
		resolve(tmp)
	})
...
async function importFile(sqlFile, file) {
	...
	let child = spawn('./lib/sqlite3.exe', [file, `.read ${sqlFile}`], {
		cwd: __dirname
	})
	let resolve
	let reject
...
async function dbfix(file, password, alg) {
	...
	if (password) {
		...
		let tmp = await exportSQL(decryptedFile)
		await importFile(tmp, decryptedFile + '.fixed')
		let encryptedFile = await encrypt(decryptedFile + '.fixed', password)
		await fse.rename(encryptedFile, file + '.encrypt.fixed')
		await fse.remove(decryptedFile)
	} else {
		let tmp = await exportSQL(file)
		await importFile(tmp, file + '.fixed')
